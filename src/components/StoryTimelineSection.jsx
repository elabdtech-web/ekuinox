...
export default function StoryTimelineSection() {
    ...
    // Smooth spring-like easing for nicer feel
    const transitionOpts = { duration: 0.7, ease: [0.16, 0.84, 0.03, 1] };
        ...
        <section className="w-full py-16 px-4 bg-[#070B13]">
            ...
            <div className="max-w-[1440px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 relative">
                ...
                <div className="flex flex-col gap-6 md:gap-56">
                    {left.map((item, idx) => {
                        // sequence: left idx -> delay = idx*2*baseDelay (so left0, right0, left1, right1, ...)
                        const seqDelay = idx * 2 * baseDelay;
                        return (
                            <motion.div
                                key={idx}
                                className="overflow-hidden mr:mr-0 md:mr-6 p-0.5 rounded-2xl shadow-sm"
                                initial={{ opacity: 0, y: 22, x: -28 }}
                                whileInView={{ opacity: 1, y: 0, x: 0 }}
                                viewport={{ once: true, amount: 0.22 }}
                                transition={{ delay: seqDelay, ...transitionOpts }}
                ...
                <div className="flex flex-col mt-0 md:mt-60 gap-6 md:gap-56">
                    {right.map((item, idx) => {
                        // sequence: right idx -> delay = idx*2*baseDelay + baseDelay (interleaved)
                        const seqDelay = idx * 2 * baseDelay + baseDelay;
                        return (
                            <motion.div
                                key={idx}
...
/* Recommended CSS for even smoother border animation (index.css):
@keyframes spin-slow {
